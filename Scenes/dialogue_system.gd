...
var current_dialogue_time : int = 0
...
func _process(delta: float) -> void:
	if current_dialogue_time == dialogue.size():
		if !player_node:
			for i in get_tree().get_nodes_in_group("Player"):
				player_node = i
			return
	...
	if next_item:
		next_item = false
		var i = dialogue[current_dialogue_time]
...
func _choice_resource(i: DialogueChoice) -> void:
	...
	for item in i.choice_text.size():
		...
		if function_resource:
			...
			DialogueButtonVar.connect("pressed", _choice_button_pressed.bind(get_node(function_resource.target_path), function_resource.wait_for_signal_to_continue), CONNECT_ONE_SHOT)
		else:
...
func _text_resource(i: DialogueText) -> void:
	...
	Dialoguelabel.visible_characters = 100
	Dialoguelabel.text = i.text
	var text_without_square_bracket: String = _text_without_square_brackets(i.text)
	var total_characters: int = text_without_square_bracket.length()
	...
	while Dialoguelabel.visible_characters < total_characters:
		...
		if character_timer >= (1.0 / i.text_speed) or text_without_square_bracket[Dialoguelabel.visible_characters] == "":
			var character: String = text_without_square_bracket[Dialoguelabel.visible_characters]
			Dialoguelabel.visible_characters += 1
			if character != "":
